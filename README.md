...
本插件支持直接选择 AstrBot 中已配置的提供商，**一键完成所有配置**！

| 配置项 | 类型 | 说明 |
|--------|------|------|
| `provider_id` | 下拉选择 | 从 AstrBot 已配置的提供商列表中选择（格式：`来源/配置名/模型名`） |

> 💡 **提示**：选择提供商后，插件会自动使用该提供商的 **API URL、API Key 和模型名称**，无需额外配置！
...
| `user_count_cap` | 整数 | `0` | 用户持有次数上限（0=不限制），达到上限后签到不再增加 |
| `api_timeout` | 整数 | `120` | API 请求等待超时时间（秒） |
...
### v1.3.6 (2026-02-24)
- ✨ **新功能**：新增 SSE 流式响应支持，兼容 `grok-imagine-1.0` 等以 `text/event-stream` 格式返回的模型。插件会自动检测响应类型，拼接所有 chunk 后提取 Markdown 图片链接并下载。

### v1.3.5 (2026-02-24)
- 🐛 **修复**：修复重启框架后提示"未选择提供商"的问题。根因是框架启动时插件 `initialize()` 早于供应商管理器初始化执行，导致 `provider` 缓存为 `None`。现改为每次调用时实时获取供应商，与框架启动顺序解耦，同时也修复了供应商重载后引用失效的问题。

### v1.3.4 (2026-02-18)
- 🐛 **修复**：修复插件加载时可能出现的 `_empty() takes no arguments` 错误，移除了事件处理函数中不兼容的参数定义。
- 🔧 **优化**：改进生图反馈提示信息，现在会完整显示 `预设 + 补充提示词`，让执行状态更清晰。

### v1.3.3 (2026-02-18)
- 🐛 **修复**：修复由于 AstrBot 核心升级导致的事件处理函数参数不匹配（TypeError）崩溃问题。
- 🔧 **优化**：为所有指令响应函数增加通用参数接收，提升系统兼容性。

### v1.3.2 (2026-02-18)
- 🐛 **修复**：修复预设指令（如 `/手办化`）后面携带的补充提示词被忽略的问题，现已支持 `指令 + 补充` 组合调用。

### v1.3.1 (2026-02-17)
- ✨ **新功能**：新增 `api_timeout` 配置项，支持自定义 API 请求等待时间（解决默认 120s 超时限制问题）。

### v1.3.0 (2026-01-25)
- 🚀 **增强**：大幅提升图片下载的成功率与稳定性。
- 🔧 **优化**：图片下载默认超时时间延长至 60 秒，并新增 3 次自动重试机制。
- ✨ **新功能**：新增图片下载 **Fallback 机制**。若图片下载因网络问题最终失败，插件将自动返回图片原始链接，确保用户仍能查看到生成结果。
- 🐛 **修复**：修复了在某些慢速网络/CDN 下图片下载超时导致插件错误报告“未找到图像数据”的逻辑 Bug。

### v1.2.1 (2026-01-07)
- 🐛 **修复**：修复 API URL 重复拼接 `/v1` 导致 404 错误的问题
- 🔧 **优化**：智能识别 API URL 格式，自动处理 `/v1` 结尾、完整路径、基础域名等不同情况

### v1.2.0 (2026-01-07)
- ✨ **新功能**：支持从 AstrBot 提供商列表中选择 API 提供商（下拉选择，自动获取 URL、Key 和模型）
- ✨ **优化**：配置界面更加直观，选择提供商后无需额外配置
